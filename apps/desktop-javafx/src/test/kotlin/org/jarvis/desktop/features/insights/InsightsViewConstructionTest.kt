...
/**
 * Headless construction coverage for the gauge/card-based [InsightsView], on
 * the Monocle software Glass/Prism pipeline (no display, no TestFX
 * interaction — only that the constructor + `init {}` that builds the
 * widget tree and loads the view-local stylesheet completes without
 * throwing). Mirrors the bootstrap used by the shared
 * `org.jarvis.desktop.headless` smoke tests, duplicated here so this
 * feature package's test coverage does not depend on editing a file
 * outside `features/insights`.
 */
class InsightsViewConstructionTest {
    ...
    @Test
    fun `InsightsView constructs its card tree and shuts down cleanly`() {
        onFxThread {
            val view = InsightsView(deadApiClient)
            assertNotNull(view)
