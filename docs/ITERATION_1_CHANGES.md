...
### P2: ImagePullBackOff
- ✅ `k8s/overlays/local/embedding-service.yaml`: изменён `imagePullPolicy: IfNotPresent` → `Never`
- ✅ `k8s/overlays/local/postgres-pgvector.yaml`: добавлен `imagePullPolicy: IfNotPresent`
...
2. `k8s/overlays/local/embedding-service.yaml` - imagePullPolicy: Never
3. `k8s/overlays/local/postgres-pgvector.yaml` - imagePullPolicy: IfNotPresent
