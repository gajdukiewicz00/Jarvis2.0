...
**Strict repo mode:**
- FAIL на активные references в `k8s/base` и `k8s/overlays/prod`
- Игнорирует закомментированные строки
...
- `k8s/overlays/prod/kustomization.yaml` не ссылается на `k8s/dev`

**Проверено:**
- `jarvis-launch.sh` не использует `k8s/dev`
- `kubectl apply -k k8s/overlays/prod` не подтягивает legacy
