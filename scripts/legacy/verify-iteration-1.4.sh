...
if [[ "$STRICT_REPO" == "true" ]]; then
    ...
    # Check k8s manifests for assistant-core references (only product paths: base and overlays/prod)
    # Exclude commented lines (starting with # or containing # Legacy)
    # Exclude RABBITMQ_RK_ASSISTANT (normal routing key, not legacy)
    ASSISTANT_CORE_REFS=$(grep -r "assistant-core\|ASSISTANT_CORE" "${PROJECT_ROOT}/k8s/base" "${PROJECT_ROOT}/k8s/overlays/prod" 2>/dev/null | grep -v "^[[:space:]]*#" | grep -v "# Legacy" | grep -v "RABBITMQ_RK_ASSISTANT" | wc -l | tr -d '\n' || echo "0")
    ...
    if [[ "$ASSISTANT_CORE_REFS" -eq 0 ]] || [[ "$ASSISTANT_CORE_REFS" == "0" ]]; then
        echo -e "  ${GREEN}✅${NC} No assistant-core references in product k8s manifests (k8s/base, k8s/overlays/prod)"
        PASSED=$((PASSED + 1))
    else
        echo -e "  ${RED}❌${NC} assistant-core references found in product k8s manifests ($ASSISTANT_CORE_REFS found)"
        FAILED=$((FAILED + 1))
        echo -e "    ${GRAY}Details:${NC}"
        grep -r "assistant-core\|ASSISTANT_CORE" "${PROJECT_ROOT}/k8s/base" "${PROJECT_ROOT}/k8s/overlays/prod" 2>/dev/null | grep -v "^[[:space:]]*#" | grep -v "# Legacy" | grep -v "RABBITMQ_RK_ASSISTANT" | head -3 | sed 's/^/      /'
    fi
else
...
# =============================================================================
# Stage 16: Local Secrets & Security (Prod-only)
# =============================================================================
echo ""
echo "=========================================="
echo "Stage 16: Local Secrets & Security"
echo "=========================================="

# Check for secrets in git (FAIL if found)
echo "Checking for secrets in repository..."

# Check for BEGIN PRIVATE KEY
PRIVATE_KEY_MATCHES=$(grep -rIn --exclude-dir=.git --exclude-dir=target --exclude-dir=.idea --exclude-dir=node_modules "BEGIN.*PRIVATE KEY" "${PROJECT_ROOT}" 2>/dev/null | grep -v ".example" | head -10 || true)
if [[ -n "$PRIVATE_KEY_MATCHES" ]]; then
    echo -e "  ${RED}❌${NC} Private keys found in repository (FAIL)"
    echo -e "    ${GRAY}Private keys should not be in git${NC}"
    echo -e "    ${GRAY}Matches:${NC}"
    echo "$PRIVATE_KEY_MATCHES" | sed 's/^/      /'
    FAILED=$((FAILED + 1))
else
    echo -e "  ${GREEN}✅${NC} No private keys found in repository"
    PASSED=$((PASSED + 1))
fi

# Check for password=, token=, aws_access_key in non-example files
SECRET_PATTERNS=(
    "password\\s*=\\s*[^\\s]+"
    "token\\s*=\\s*[^\\s]+"
    "aws_access_key"
...
for pattern in "${SECRET_PATTERNS[@]}"; do
    MATCHES=$(grep -rInE --exclude-dir=.git --exclude-dir=target --exclude-dir=.idea --exclude-dir=node_modules --exclude="*.example.*" --exclude="*.example" "$pattern" "${PROJECT_ROOT}/k8s" "${PROJECT_ROOT}/scripts" "${PROJECT_ROOT}/apps" 2>/dev/null | grep -v "CHANGE-ME\|example\|template\|TODO\|FIXME" | head -10 || true)
    if [[ -n "$MATCHES" ]]; then
...
if [[ $SECRET_VIOLATIONS -gt 0 ]]; then
    echo -e "  ${RED}❌${NC} Potential secrets found in repository (password=, token=, etc.)"
    echo -e "    ${GRAY}Secrets should not be in git (use secrets.example.env template)${NC}"
    FAILED=$((FAILED + 1))
else
    echo -e "  ${GREEN}✅${NC} No obvious secrets found in repository"
    PASSED=$((PASSED + 1))
fi

# Check for .env files without .example
ENV_FILES=$(find "${PROJECT_ROOT}" -name ".env" -o -name "*.env" 2>/dev/null | grep -v ".example" | grep -v "node_modules" | grep -v "target" | head -10 || true)
if [[ -n "$ENV_FILES" ]]; then
    echo -e "  ${RED}❌${NC} .env files found without .example suffix (FAIL)"
    echo -e "    ${GRAY}Real .env files should not be in git${NC}"
    echo "$ENV_FILES" | sed 's/^/      /'
    FAILED=$((FAILED + 1))
else
    echo -e "  ${GREEN}✅${NC} No .env files found (only .example templates)"
    PASSED=$((PASSED + 1))
fi

# Check for secrets*.yaml not in .gitignore (should be ignored)
if [[ -f "${PROJECT_ROOT}/.gitignore" ]]; then
    if grep -q "^secrets/" "${PROJECT_ROOT}/.gitignore" 2>/dev/null || grep -q "^secrets\*" "${PROJECT_ROOT}/.gitignore" 2>/dev/null; then
        echo -e "  ${GREEN}✅${NC} secrets/ directory in .gitignore"
        PASSED=$((PASSED + 1))
    else
        echo -e "  ${YELLOW}⚠️${NC}  secrets/ may not be in .gitignore"
        WARNINGS=$((WARNINGS + 1))
    fi
fi

# Check for secrets*.yaml files in git (should not exist)
SECRETS_YAML=$(find "${PROJECT_ROOT}/k8s" -name "secrets*.yaml" 2>/dev/null | grep -v ".example" | grep -v "legacy" | head -10 || true)
if [[ -n "$SECRETS_YAML" ]]; then
    echo -e "  ${RED}❌${NC} secrets*.yaml files found in k8s/ (should not be in git)"
    echo -e "    ${GRAY}Files:${NC}"
    echo "$SECRETS_YAML" | sed 's/^/      /'
    echo -e "    ${GRAY}Action: Remove or move to legacy/, use jarvis-secrets-apply.sh instead${NC}"
    FAILED=$((FAILED + 1))
else
    echo -e "  ${GREEN}✅${NC} No secrets*.yaml files in k8s/ (except legacy)"
    PASSED=$((PASSED + 1))
fi

# Check for k8s/base/secrets.yaml with real values (should be removed or empty)
if [[ -f "${PROJECT_ROOT}/k8s/base/secrets.yaml" ]]; then
    if grep -q "CHANGE-ME\|jarvis-k8s-password-change-me\|jarvis-dev-secret" "${PROJECT_ROOT}/k8s/base/secrets.yaml" 2>/dev/null; then
        echo -e "  ${YELLOW}⚠️${NC}  k8s/base/secrets.yaml contains placeholder values (should be removed in Stage 16)"
        echo -e "    ${GRAY}Action: Remove secrets.yaml from git, use jarvis-secrets-apply.sh${NC}"
        WARNINGS=$((WARNINGS + 1))
    else
        echo -e "  ${GREEN}✅${NC} k8s/base/secrets.yaml does not contain real secrets"
        PASSED=$((PASSED + 1))
    fi
fi

# Check secrets in cluster (if --require-backend)
if [[ "$REQUIRE_BACKEND" == "true" ]]; then
    echo ""
    echo "Checking secrets in Kubernetes cluster..."
    
    if kubectl get secret jarvis-secrets -n jarvis >/dev/null 2>&1; then
        echo -e "  ${GREEN}✅${NC} jarvis-secrets exists in cluster"
        PASSED=$((PASSED + 1))
        
        # Check for required keys
        REQUIRED_KEYS=("POSTGRES_USER" "POSTGRES_PASSWORD" "JWT_SECRET" "SPRING_DATASOURCE_USERNAME" "SPRING_DATASOURCE_PASSWORD")
        SECRET_KEYS=$(kubectl get secret jarvis-secrets -n jarvis -o jsonpath='{.data}' 2>/dev/null | grep -o '"[^"]*":' | tr -d '":' || echo "")
        ...
        if [[ ${#MISSING_KEYS[@]} -gt 0 ]]; then
            echo -e "  ${RED}❌${NC} Missing required keys in jarvis-secrets: ${MISSING_KEYS[*]}"
            FAILED=$((FAILED + 1))
        else
            echo -e "  ${GREEN}✅${NC} All required keys present in jarvis-secrets"
            PASSED=$((PASSED + 1))
        fi
    else
        echo -e "  ${RED}❌${NC} jarvis-secrets not found in cluster"
        echo -e "    ${GRAY}Action: Run ./scripts/product/jarvis-secrets-apply.sh${NC}"
        FAILED=$((FAILED + 1))
    fi
else
    echo -e "  ${GRAY}⏭️${NC}  Cluster secrets check skipped (use --require-backend to enforce)"
fi
