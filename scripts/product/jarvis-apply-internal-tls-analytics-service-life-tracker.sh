...
usage() {
  cat <<'EOF'
Usage: ./scripts/product/jarvis-apply-internal-tls-analytics-service-life-tracker.sh [--namespace=jarvis-prod]
...
SLICE_DIR="${JARVIS_HOME}/tls/internal/analytics-service-life-tracker"
...
if [[ ! -f "${TRUSTSTORE}" || ! -f "${CA_CERT}" ]]; then
  "${PROJECT_ROOT}/scripts/product/jarvis-generate-internal-tls-analytics-service-life-tracker.sh"
fi
