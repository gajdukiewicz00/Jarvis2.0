...
SLICE_DIR="${JARVIS_HOME}/tls/internal/planner-service-analytics-service"
...
if [[ ! -f "${TRUSTSTORE}" || ! -f "${CA_CERT}" ]]; then
  "${PROJECT_ROOT}/scripts/product/jarvis-generate-internal-tls-planner-service-analytics-service.sh"
fi
