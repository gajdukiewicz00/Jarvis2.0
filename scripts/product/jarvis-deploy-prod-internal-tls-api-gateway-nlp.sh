...
OVERLAY_PATH="${PROJECT_ROOT}/k8s/overlays/prod-release-internal-tls-api-gateway-nlp"
...
require_secret_key() {
  ...
  if [[ -z "${value}" ]]; then
    ...
    echo "   Refresh it with ./scripts/product/jarvis-apply-internal-tls-api-gateway-nlp.sh" >&2
    exit 1
  fi
}
...
if ! kubectl -n "${NAMESPACE}" get secret "${SECRET_NAME}" >/dev/null 2>&1; then
  ...
  echo "   Create it with ./scripts/product/jarvis-apply-internal-tls-api-gateway-nlp.sh" >&2
  exit 1
fi
...
export K8S_PREFLIGHT_REQUIRED_SECRETS="${K8S_PREFLIGHT_REQUIRED_SECRETS:-jarvis-secrets,jarvis-tls,jarvis-internal-tls-api-gateway-nlp}"
