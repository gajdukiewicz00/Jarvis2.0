...
OVERLAY_PATH="${PROJECT_ROOT}/k8s/overlays/prod-release-internal-tls-api-gateway-pc-control"
NAMESPACE="${JARVIS_NAMESPACE:-jarvis-prod}"
SECRET_NAME="jarvis-internal-tls-api-gateway-pc-control"
REQUIRED_SECRET_KEYS=(
  pc-control-keystore.p12
  ca.crt
  keystore-password
)
...
require_secret_key() {
  ...
  if [[ -z "${value}" ]]; then
    ...
    echo "   Refresh it with ./scripts/product/jarvis-apply-internal-tls-api-gateway-pc-control.sh" >&2
    exit 1
  fi
}
...
if ! kubectl -n "${NAMESPACE}" get secret "${SECRET_NAME}" >/dev/null 2>&1; then
  ...
  echo "   Create it with ./scripts/product/jarvis-apply-internal-tls-api-gateway-pc-control.sh" >&2
  exit 1
fi
