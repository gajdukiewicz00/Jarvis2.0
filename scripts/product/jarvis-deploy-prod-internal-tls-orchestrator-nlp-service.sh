...
OVERLAY_PATH="${PROJECT_ROOT}/k8s/overlays/prod-release-internal-tls-orchestrator-nlp-service"
...
require_secret_key() {
  ...
  if [[ -z "${value}" ]]; then
    ...
    echo "   Refresh it with ./scripts/product/jarvis-apply-internal-tls-orchestrator-nlp-service.sh" >&2
    exit 1
  fi
}
