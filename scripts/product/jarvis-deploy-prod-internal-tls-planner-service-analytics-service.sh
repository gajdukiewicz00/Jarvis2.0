...
OVERLAY_PATH="${PROJECT_ROOT}/k8s/overlays/prod-release-internal-tls-planner-service-analytics-service"
...
REQUIRED_SECRETS=(
  ...
  jarvis-internal-tls-orchestrator-nlp-service
  jarvis-internal-tls-planner-service-analytics-service
)
...
require_secret_key() {
  ...
  if [[ -z "${value}" ]]; then
    ...
    echo "   Refresh it with ./scripts/product/jarvis-apply-internal-tls-planner-service-analytics-service.sh" >&2
    exit 1
  fi
}
