...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      cat <<'EOF'
Usage: ./scripts/product/jarvis-generate-internal-tls-api-gateway-analytics-service.sh [--force]

Generates the narrow internal TLS material for the seventh migrated hop:
  api-gateway (HTTPS client) -> analytics-service (HTTPS server)
EOF
      exit 0
      ;;
    *)
...
SLICE_DIR="${TLS_ROOT}/internal/api-gateway-analytics-service"
CA_KEY="${TLS_ROOT}/jarvis-ca.key"
CA_CERT="${TLS_ROOT}/jarvis-ca.crt"

SERVER_KEY="${SLICE_DIR}/analytics-service.key"
SERVER_CSR="${SLICE_DIR}/analytics-service.csr"
SERVER_CERT="${SLICE_DIR}/analytics-service.crt"
SERVER_KEYSTORE="${SLICE_DIR}/analytics-service-keystore.p12"
CA_CERT_COPY="${SLICE_DIR}/ca.crt"

KEYSTORE_PASSWORD="${JARVIS_INTERNAL_TLS_API_GATEWAY_ANALYTICS_SERVICE_KEYSTORE_PASSWORD:-changeit}"
...
[ dn ]
CN = analytics-service.jarvis-prod.svc.cluster.local
O  = Jarvis
...
[ alt_names ]
DNS.1 = analytics-service
DNS.2 = analytics-service.jarvis-prod.svc.cluster.local
DNS.3 = localhost
IP.1 = 127.0.0.1
EOF
