...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      cat <<'EOF'
Usage: ./scripts/product/jarvis-generate-internal-tls-api-gateway-life-tracker.sh [--force]

Generates the narrow internal TLS material for the ninth migrated hop:
  api-gateway (HTTPS client) -> life-tracker (HTTPS server)
EOF
      exit 0
      ;;
    *)
...
SLICE_DIR="${TLS_ROOT}/internal/api-gateway-life-tracker"
CA_KEY="${TLS_ROOT}/jarvis-ca.key"
CA_CERT="${TLS_ROOT}/jarvis-ca.crt"

SERVER_KEY="${SLICE_DIR}/life-tracker.key"
SERVER_CSR="${SLICE_DIR}/life-tracker.csr"
SERVER_CERT="${SLICE_DIR}/life-tracker.crt"
SERVER_KEYSTORE="${SLICE_DIR}/life-tracker-keystore.p12"
CA_CERT_COPY="${SLICE_DIR}/ca.crt"

KEYSTORE_PASSWORD="${JARVIS_INTERNAL_TLS_API_GATEWAY_LIFE_TRACKER_KEYSTORE_PASSWORD:-changeit}"
...
[ dn ]
CN = life-tracker.jarvis-prod.svc.cluster.local
O  = Jarvis
...
[ alt_names ]
DNS.1 = life-tracker
DNS.2 = life-tracker.jarvis-prod.svc.cluster.local
DNS.3 = localhost
IP.1 = 127.0.0.1
EOF
