...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      cat <<'EOF'
Usage: ./scripts/product/jarvis-generate-internal-tls-api-gateway-nlp.sh [--force]

Generates the narrow internal TLS material for the first migrated slice:
...
SLICE_DIR="${TLS_ROOT}/internal/api-gateway-nlp"
CA_KEY="${TLS_ROOT}/jarvis-ca.key"
CA_CERT="${TLS_ROOT}/jarvis-ca.crt"
CA_SERIAL="${TLS_ROOT}/jarvis-ca.srl"

SERVER_KEY="${SLICE_DIR}/nlp-service.key"
SERVER_CSR="${SLICE_DIR}/nlp-service.csr"
SERVER_CERT="${SLICE_DIR}/nlp-service.crt"
