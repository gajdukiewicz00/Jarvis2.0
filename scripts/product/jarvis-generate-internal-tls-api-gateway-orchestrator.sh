...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      cat <<'EOF'
Usage: ./scripts/product/jarvis-generate-internal-tls-api-gateway-orchestrator.sh [--force]

Generates the narrow internal TLS material for the eleventh migrated hop:
...
SLICE_DIR="${TLS_ROOT}/internal/api-gateway-orchestrator"
CA_KEY="${TLS_ROOT}/jarvis-ca.key"
CA_CERT="${TLS_ROOT}/jarvis-ca.crt"

SERVER_KEY="${SLICE_DIR}/orchestrator.key"
SERVER_CSR="${SLICE_DIR}/orchestrator.csr"
SERVER_CERT="${SLICE_DIR}/orchestrator.crt"
...
[ dn ]
CN = orchestrator.jarvis-prod.svc.cluster.local
O  = Jarvis
...
[ alt_names ]
DNS.1 = orchestrator
DNS.2 = orchestrator.jarvis-prod.svc.cluster.local
DNS.3 = localhost
IP.1 = 127.0.0.1
EOF
