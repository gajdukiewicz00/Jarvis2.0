...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      cat <<'EOF'
Usage: ./scripts/product/jarvis-generate-internal-tls-api-gateway-pc-control.sh [--force]

Generates the narrow internal TLS material for the eighth migrated hop:
...
SLICE_DIR="${TLS_ROOT}/internal/api-gateway-pc-control"
CA_KEY="${TLS_ROOT}/jarvis-ca.key"
CA_CERT="${TLS_ROOT}/jarvis-ca.crt"

SERVER_KEY="${SLICE_DIR}/pc-control.key"
SERVER_CSR="${SLICE_DIR}/pc-control.csr"
SERVER_CERT="${SLICE_DIR}/pc-control.crt"
...
[ dn ]
CN = pc-control.jarvis-prod.svc.cluster.local
O  = Jarvis
...
[ alt_names ]
DNS.1 = pc-control
DNS.2 = pc-control.jarvis-prod.svc.cluster.local
DNS.3 = localhost
IP.1 = 127.0.0.1
EOF
