...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      cat <<'EOF'
Usage: ./scripts/product/jarvis-generate-internal-tls-api-gateway-security-service.sh [--force]

Generates the narrow internal TLS material for the sixth migrated hop:
  api-gateway (HTTPS client) -> security-service (HTTPS server)
EOF
      exit 0
      ;;
    *)
...
SLICE_DIR="${TLS_ROOT}/internal/api-gateway-security-service"
CA_KEY="${TLS_ROOT}/jarvis-ca.key"
CA_CERT="${TLS_ROOT}/jarvis-ca.crt"

SERVER_KEY="${SLICE_DIR}/security-service.key"
SERVER_CSR="${SLICE_DIR}/security-service.csr"
SERVER_CERT="${SLICE_DIR}/security-service.crt"
SERVER_KEYSTORE="${SLICE_DIR}/security-service-keystore.p12"
CA_CERT_COPY="${SLICE_DIR}/ca.crt"

KEYSTORE_PASSWORD="${JARVIS_INTERNAL_TLS_API_GATEWAY_SECURITY_SERVICE_KEYSTORE_PASSWORD:-changeit}"
...
[ dn ]
CN = security-service.jarvis-prod.svc.cluster.local
O  = Jarvis
...
[ alt_names ]
DNS.1 = security-service
DNS.2 = security-service.jarvis-prod.svc.cluster.local
DNS.3 = localhost
IP.1 = 127.0.0.1
EOF
