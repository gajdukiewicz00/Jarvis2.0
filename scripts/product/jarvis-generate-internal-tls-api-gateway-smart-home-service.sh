...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      cat <<'EOF'
Usage: ./scripts/product/jarvis-generate-internal-tls-api-gateway-smart-home-service.sh [--force]

Generates the narrow internal TLS material for the tenth migrated hop:
...
SLICE_DIR="${TLS_ROOT}/internal/api-gateway-smart-home-service"
CA_KEY="${TLS_ROOT}/jarvis-ca.key"
CA_CERT="${TLS_ROOT}/jarvis-ca.crt"

SERVER_KEY="${SLICE_DIR}/smart-home-service.key"
SERVER_CSR="${SLICE_DIR}/smart-home-service.csr"
SERVER_CERT="${SLICE_DIR}/smart-home-service.crt"
