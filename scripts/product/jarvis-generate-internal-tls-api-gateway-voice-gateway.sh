...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      cat <<'EOF'
Usage: ./scripts/product/jarvis-generate-internal-tls-api-gateway-voice-gateway.sh [--force]

Generates the narrow internal TLS material for the twelfth migrated hop:
...
SLICE_DIR="${TLS_ROOT}/internal/api-gateway-voice-gateway"
CA_KEY="${TLS_ROOT}/jarvis-ca.key"
CA_CERT="${TLS_ROOT}/jarvis-ca.crt"

SERVER_KEY="${SLICE_DIR}/voice-gateway.key"
SERVER_CSR="${SLICE_DIR}/voice-gateway.csr"
SERVER_CERT="${SLICE_DIR}/voice-gateway.crt"
