...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      cat <<'EOF'
Usage: ./scripts/product/jarvis-generate-internal-tls-orchestrator-api-gateway.sh [--force]

Generates the dedicated trust material for the third migrated slice:
...
SLICE_DIR="${TLS_ROOT}/internal/orchestrator-api-gateway"
TRUSTSTORE="${SLICE_DIR}/jarvis-internal-truststore.jks"
CA_CERT_COPY="${SLICE_DIR}/ca.crt"
TRUSTSTORE_PASSWORD="${JARVIS_INTERNAL_TLS_ORCHESTRATOR_API_GATEWAY_TRUSTSTORE_PASSWORD:-changeit}"
