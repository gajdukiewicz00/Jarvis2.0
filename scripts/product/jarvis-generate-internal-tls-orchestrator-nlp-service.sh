...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      cat <<'EOF'
Usage: ./scripts/product/jarvis-generate-internal-tls-orchestrator-nlp-service.sh [--force]
...
SOURCE_DIR="${TLS_ROOT}/internal/api-gateway-nlp"
SOURCE_CA_CERT="${SOURCE_DIR}/ca.crt"
SLICE_DIR="${TLS_ROOT}/internal/orchestrator-nlp-service"
...
if [[ ! -f "${SOURCE_CA_CERT}" ]]; then
  echo "❌ Missing source CA for nlp-service internal HTTPS listener: ${SOURCE_CA_CERT}" >&2
  exit 1
fi
