...
for arg in "$@"; do
  case "${arg}" in
    --help|-h)
      cat <<'EOF'
Usage: ./scripts/product/jarvis-generate-internal-tls-orchestrator-pc-control.sh

Generates the narrow internal TLS trust material for:
  orchestrator (HTTPS client) -> pc-control (existing HTTPS server on 8084)
EOF
      exit 0
      ;;
    *)
...
SLICE_DIR="${TLS_ROOT}/internal/orchestrator-pc-control"
