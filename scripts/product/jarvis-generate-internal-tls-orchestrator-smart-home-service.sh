...
for arg in "$@"; do
  case "${arg}" in
    --help|-h)
      cat <<'EOF'
Usage: ./scripts/product/jarvis-generate-internal-tls-orchestrator-smart-home-service.sh

Generates the narrow internal TLS trust material for:
  orchestrator (HTTPS client) -> smart-home-service (existing HTTPS server on 8086)
EOF
      exit 0
      ;;
    *)
...
SLICE_DIR="${TLS_ROOT}/internal/orchestrator-smart-home-service"
