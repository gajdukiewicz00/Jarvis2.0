...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      cat <<'EOF'
Usage: ./scripts/product/jarvis-generate-internal-tls-planner-api-gateway.sh [--force]

Generates the narrow internal TLS material for the second migrated slice:
...
SLICE_DIR="${TLS_ROOT}/internal/planner-api-gateway"
CA_KEY="${TLS_ROOT}/jarvis-ca.key"
CA_CERT="${TLS_ROOT}/jarvis-ca.crt"

SERVER_KEY="${SLICE_DIR}/api-gateway-internal.key"
SERVER_CSR="${SLICE_DIR}/api-gateway-internal.csr"
SERVER_CERT="${SLICE_DIR}/api-gateway-internal.crt"
...
if [[ "${FORCE}" == "true" ]]; then
  rm -f "${SERVER_KEY}" "${SERVER_CSR}" "${SERVER_CERT}" "${SERVER_KEYSTORE}" "${TRUSTSTORE}" "${CA_CERT_COPY}"
fi
...
if [[ ! -f "${TRUSTSTORE}" || "${CA_CERT}" -nt "${TRUSTSTORE}" ]]; then
  rm -f "${TRUSTSTORE}"
