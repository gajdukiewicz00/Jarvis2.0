...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      cat <<'EOF'
Usage: ./scripts/product/jarvis-generate-internal-tls-planner-service-analytics-service.sh [--force]
...
SOURCE_DIR="${TLS_ROOT}/internal/api-gateway-analytics-service"
SOURCE_CA_CERT="${SOURCE_DIR}/ca.crt"
SLICE_DIR="${TLS_ROOT}/internal/planner-service-analytics-service"
TRUSTSTORE="${SLICE_DIR}/jarvis-internal-truststore.jks"
CA_CERT_COPY="${SLICE_DIR}/ca.crt"
TRUSTSTORE_PASSWORD="${JARVIS_INTERNAL_TLS_PLANNER_SERVICE_ANALYTICS_SERVICE_TRUSTSTORE_PASSWORD:-changeit}"
...
if [[ ! -f "${SOURCE_CA_CERT}" ]]; then
  echo "❌ Missing source CA for analytics-service internal HTTPS listener: ${SOURCE_CA_CERT}" >&2
  exit 1
fi
