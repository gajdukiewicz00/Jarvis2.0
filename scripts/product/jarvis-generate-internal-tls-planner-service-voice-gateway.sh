...
for arg in "$@"; do
  case "${arg}" in
    --help|-h)
      cat <<'EOF'
Usage: ./scripts/product/jarvis-generate-internal-tls-planner-service-voice-gateway.sh

Generates the narrow internal TLS trust material for:
  planner-service (HTTPS client) -> voice-gateway (existing HTTPS server on 8081)
EOF
      exit 0
      ;;
    *)
...
SLICE_DIR="${TLS_ROOT}/internal/planner-service-voice-gateway"
