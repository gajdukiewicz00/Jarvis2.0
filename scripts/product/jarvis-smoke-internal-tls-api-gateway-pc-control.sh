...
Validates the eighth internal TLS hop:
  ...
  2. api-gateway completes a real pc-control action flow against pc-control over HTTPS
EOF
}
...
kubectl -n "${NAMESPACE}" port-forward service/pc-control "${PC_CONTROL_PORT}:8084" >"${tmp_dir}/pc-control-port-forward.log" 2>&1 &
pc_control_pf_pid=$!
...
wait_for_port_forward "https://127.0.0.1:${PC_CONTROL_PORT}/actuator/health" -k

curl --fail --silent --show-error \
  --cacert "${ca_file}" \
  "https://127.0.0.1:${PC_CONTROL_PORT}/actuator/health/readiness" >/dev/null
echo "✅ pc-control readiness responded over HTTPS"

if curl --fail --silent --show-error "http://127.0.0.1:${PC_CONTROL_PORT}/actuator/health/readiness" >/dev/null 2>&1; then
  echo "❌ pc-control still answered plain HTTP on the internal TLS slice" >&2
  exit 1
fi
echo "✅ pc-control rejected plain HTTP on the migrated internal port"

smoke_username="gateway-pc-control-tls-$(date +%s)"
auth_response_file="${tmp_dir}/gateway-auth-response.json"
curl --fail --silent --show-error \
  -H 'Content-Type: application/json' \
  -d "{\"username\":\"${smoke_username}\",\"password\":\"GatewayPcControlTls!123\",\"role\":\"USER\"}" \
...
echo "✅ api-gateway auth proxy returned accessToken for pc-control smoke"

smoke_run_id="pc-control-hop-$(date +%s)"
pc_response_file="${tmp_dir}/gateway-pc-control-response.json"
curl --fail --silent --show-error \
  -H "Authorization: Bearer ${access_token}" \
  -H "X-Smoke-Run-Id: ${smoke_run_id}" \
  -H "Content-Type: application/json" \
  -d "{\"actionType\":\"NOTIFY\",\"parameters\":{\"title\":\"TLS Smoke ${smoke_run_id}\",\"message\":\"pc-control hop\"}}" \
  "http://127.0.0.1:${API_GATEWAY_PORT}/api/v1/pc/action" > "${pc_response_file}"

python3 - <<'PY' "${pc_response_file}"
import json
import pathlib
import sys

payload = json.loads(pathlib.Path(sys.argv[1]).read_text())
if payload.get("actionType") != "NOTIFY":
    raise SystemExit(f"pc-control response did not contain actionType=NOTIFY: {payload}")
if payload.get("status") not in {"SUCCESS", "PARTIAL_SUCCESS"}:
    raise SystemExit(f"pc-control response did not contain successful status: {payload}")
print("✅ api-gateway pc-control proxy returned a successful NOTIFY result from pc-control")
PY
...
for _ in $(seq 1 10); do
  ...
  if printf '%s\n' "${api_gateway_logs}" | rg -q "${smoke_run_id}" \
    && printf '%s\n' "${api_gateway_logs}" | rg -q 'https://pc-control\.jarvis\.svc\.cluster\.local:8084'; then
    break
  fi
  sleep 1
done
