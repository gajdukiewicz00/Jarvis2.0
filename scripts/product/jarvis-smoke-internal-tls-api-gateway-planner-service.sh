...
Validates the seventeenth internal TLS hop:
  ...
  2. api-gateway completes a real tool list flow against planner-service over HTTPS
EOF
}
...
kubectl -n "${NAMESPACE}" port-forward service/planner-service "${PLANNER_SERVICE_PORT}:8092" >"${tmp_dir}/planner-port-forward.log" 2>&1 &
planner_pf_pid=$!
...
curl --fail --silent --show-error \
  --cacert "${ca_file}" \
  "https://127.0.0.1:${PLANNER_SERVICE_PORT}/actuator/health/readiness" >/dev/null
echo "✅ planner-service readiness responded over HTTPS"

if curl --fail --silent --show-error "http://127.0.0.1:${PLANNER_SERVICE_PORT}/actuator/health/readiness" >/dev/null 2>&1; then
  echo "❌ planner-service still answered plain HTTP on the internal TLS slice" >&2
  exit 1
fi
echo "✅ planner-service rejected plain HTTP on the migrated internal port"

smoke_username="gateway-planner-tls-$(date +%s)"
auth_response_file="${tmp_dir}/gateway-auth-response.json"
curl --fail --silent --show-error \
  -H 'Content-Type: application/json' \
  -d "{\"username\":\"${smoke_username}\",\"password\":\"GatewayPlannerTls!123\",\"role\":\"USER\"}" \
...
echo "✅ api-gateway auth proxy returned accessToken for planner smoke"

planner_user_id="$(python3 - <<'PY' "${access_token}"
import base64
import json
import sys
...
if not subject:
    raise SystemExit(f"planner smoke access token did not contain sub: {payload}")
print(subject)
PY
)"

planner_response_file="${tmp_dir}/gateway-planner-response.json"
curl --fail --silent --show-error \
  -H "Authorization: Bearer ${access_token}" \
  -H 'Content-Type: application/json' \
  --data '{}' \
  "http://127.0.0.1:${API_GATEWAY_PORT}/api/v1/tools/todo/list" > "${planner_response_file}"

python3 - <<'PY' "${planner_response_file}"
import json
import pathlib
import sys
...
if not isinstance(payload, list):
    raise SystemExit(f"planner tool list response was not a JSON list: {payload!r}")
print("✅ api-gateway planner proxy returned a JSON todo list from planner-service")
PY
...
for _ in $(seq 1 10); do
  ...
  if printf '%s\n' "${api_gateway_logs}" | rg -q "userId=${planner_user_id}" \
    && printf '%s\n' "${api_gateway_logs}" | rg -q 'https://planner-service\.jarvis\.svc\.cluster\.local:8092'; then
    break
  fi
  sleep 1
done

if ! printf '%s\n' "${api_gateway_logs}" | rg -q "userId=${planner_user_id}"; then
  echo "❌ api-gateway logs did not include the delegated planner smoke userId ${planner_user_id}" >&2
  exit 1
fi
if ! printf '%s\n' "${api_gateway_logs}" | rg -q 'https://planner-service\.jarvis\.svc\.cluster\.local:8092'; then
  echo "❌ api-gateway logs did not show HTTPS planner-service routing on port 8092" >&2
  exit 1
fi
...
if ! printf '%s\n' "${planner_logs}" | rg -q "userId=${planner_user_id}"; then
  echo "❌ planner-service logs did not include the delegated planner smoke userId ${planner_user_id}" >&2
  exit 1
fi
