...
TLS_SECRET_NAME="${JARVIS_TLS_SLICE_TLS_SECRET_NAME:-jarvis-internal-tls-orchestrator-api-gateway}"
...
Validates the third internal TLS slice:
  ...
  2. orchestrator reaches api-gateway over HTTPS on the migrated path
EOF
}
...
smoke_subject="orchestrator-tls-smoke-$(date +%s)"
service_token="$(
...
orchestrator_url="http://127.0.0.1:${ORCHESTRATOR_SERVICE_PORT}"
orchestrator_curl_args=()
if curl --fail --silent --show-error \
  --cacert "${ca_file}" \
  "https://127.0.0.1:${ORCHESTRATOR_SERVICE_PORT}/actuator/health/readiness" >/dev/null 2>&1; then
  orchestrator_url="https://127.0.0.1:${ORCHESTRATOR_SERVICE_PORT}"
  orchestrator_curl_args+=(--cacert "${ca_file}")
  echo "✅ orchestrator service responds over HTTPS on 8083"
else
  wait_for_port_forward "http://127.0.0.1:${ORCHESTRATOR_SERVICE_PORT}/actuator/health/readiness"
  echo "✅ orchestrator service responds over HTTP on 8083"
fi

response_file="${tmp_dir}/orchestrator-execute-response.txt"
curl --fail --silent --show-error \
  "${orchestrator_curl_args[@]}" \
  -X POST \
  -H "Authorization: Bearer ${service_token}" \
  -H 'Content-Type: application/json' \
  --data "{\"intent\":\"volume_up\",\"parameters\":{\"delta\":\"17\"},\"language\":\"ru\",\"correlationId\":\"${smoke_subject}\",\"originalText\":\"tls smoke\"}" \
  "${orchestrator_url}/api/v1/orchestrator/execute" \
  > "${response_file}"
...
print("✅ orchestrator execute endpoint returned a non-empty response")
PY

orchestrator_logs="$(kubectl -n "${NAMESPACE}" logs deploy/orchestrator --since=2m 2>/dev/null || true)"
if ! printf '%s\n' "${orchestrator_logs}" | rg -q "${smoke_subject}"; then
  echo "❌ orchestrator logs did not include the unique smoke correlation id ${smoke_subject}" >&2
  exit 1
fi
if ! printf '%s\n' "${orchestrator_logs}" | rg -q 'https://api-gateway\.jarvis\.svc\.cluster\.local:8443'; then
  echo "❌ orchestrator logs did not show HTTPS api-gateway routing on port 8443" >&2
  exit 1
fi
if ! printf '%s\n' "${orchestrator_logs}" | rg -q 'API Gateway PC action routed'; then
  echo "❌ orchestrator logs did not record successful API Gateway PC action routing" >&2
  exit 1
fi
