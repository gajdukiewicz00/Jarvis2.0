...
TLS_SECRET_NAME="${JARVIS_TLS_SLICE_TLS_SECRET_NAME:-jarvis-internal-tls-orchestrator-smart-home-service}"
...
Validates the narrow internal TLS slice:
  ...
  2. orchestrator routes an internal smart-home action to smart-home-service over HTTPS
EOF
}
...
smoke_user="orchestrator-smart-home-tls-$(date +%s)"
service_token="$(
...
payload = {
    ...
    "exp": now + 300,
    "token_type": "service",
    "svc": "orchestrator-smart-home-tls-smoke",
    "roles": ["SVC_INTERNAL"],
}
segments = [
...
response_file="${tmp_dir}/orchestrator-smart-home-response.json"
curl --fail --silent --show-error \
  --cacert "${ca_file}" \
  -X POST \
  ...
  "https://127.0.0.1:${ORCHESTRATOR_SERVICE_PORT}/internal/orchestrator/smart-home-action" \
  > "${response_file}"
...
if payload.get("status") != "dispatched":
    raise SystemExit(f"orchestrator smart-home response unexpected: {payload}")
print("✅ orchestrator internal smart-home endpoint returned status=dispatched")
PY
...
for _ in $(seq 1 10); do
  ...
  if printf '%s\n' "${orchestrator_logs}" | rg -q "${smoke_user}" \
    && printf '%s\n' "${orchestrator_logs}" | rg -q 'https://smart-home-service\.jarvis\.svc\.cluster\.local:8086'; then
    break
  fi
  sleep 1
done
...
if ! printf '%s\n' "${orchestrator_logs}" | rg -q "${smoke_user}"; then
  echo "❌ orchestrator logs did not include the unique smart-home smoke user ${smoke_user}" >&2
  exit 1
fi
if ! printf '%s\n' "${orchestrator_logs}" | rg -q 'https://smart-home-service\.jarvis\.svc\.cluster\.local:8086'; then
  echo "❌ orchestrator logs did not show HTTPS smart-home routing on port 8086" >&2
  exit 1
fi
...
if ! printf '%s\n' "${smart_home_logs}" | rg -q "${smoke_user}"; then
  echo "❌ smart-home-service logs did not include the unique orchestrator smoke user ${smoke_user}" >&2
  exit 1
fi
if ! printf '%s\n' "${smart_home_logs}" | rg -q 'kitchen_light'; then
